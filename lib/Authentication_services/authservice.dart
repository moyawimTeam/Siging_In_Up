...
class AuthService {
  ...
  handleAuth() {
    return StreamBuilder(
        ...
        builder: (BuildContext context, snapshot) {
          if (snapshot.hasData) {
            return Test(); //Here is the desired Page
          } else {
  ...
  Future<void> verifyPhone(BuildContext context, String signInType, phoneNo,
      ...
      [firstname, lastname, phonenb, job, city, desc]) async {
    final PhoneVerificationCompleted verified =
      ...
      if (signInType == 'login') {
        ...
        AuthService()
            .createEmployer(firstname, lastname, phonenb, authCredential);
      } else if (signInType == 'signupEmpl') {
        AuthService().createEmployee(
            firstname, lastname, phonenb, job, city, desc, authCredential);
      }
      //This one for popping the popUp box
      Navigator.pop(context);
    };
